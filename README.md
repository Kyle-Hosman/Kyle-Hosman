...
I ship software end-to-end, design, build, release, and everything after. Most of my work sits where user-facing polish meets low-level systems: SwiftUI apps backed by CloudKit sync and widgets, real-time C++ audio engines where a few milliseconds of latency is the difference between usable and useless, and hardware integration that involves reading protocols nobody documented. I build the tools I want to exist, then ship them.
...
`Swift` `SwiftUI` `C++` `JUCE` `Python` `MIDI` `Ableton Link` `VST3` `Reverse engineering` `Embedded Linux`
